...
type migrationDriver interface {
	...
	MigrateUp(ctx context.Context, uploadID int, tx *basestore.Store, rows *sql.Rows) ([][]any, error)
	MigrateDown(ctx context.Context, uploadID int, tx *basestore.Store, rows *sql.Rows) ([][]any, error)
}

type driverFunc func(ctx context.Context, uploadID int, tx *basestore.Store, rows *sql.Rows) ([][]any, error)
...
//
// Code intelligence tables are very large and using a full table scan count is too expensive
...
//
//  1. There is an index on T.upload_id
//
//  2. For each distinct upload_id in table T, there is a corresponding row in table
...
// that may still need migrating. This set allows us to use the upload_id index on the target
...
WITH
	...
	ups AS (
		...
		SELECT upload_id, min_schema_version, max_schema_version
		FROM current_bounds
		WHERE
...
func (m *migrator) processRows(ctx context.Context, tx *basestore.Store, uploadID, version int, driverFunc driverFunc) (_ <-chan []any, err error) {
	...
	values, err := driverFunc(ctx, uploadID, tx, rows)
	if err != nil {
		return nil, err
	}

	rowValues := make(chan []any, len(values))
...
SELECT %s FROM %s WHERE upload_id = %s AND schema_version = %s LIMIT %s
`
