...
JOIN codeintel_scip_symbols_lookup_leaves ll ON ll.upload_id = l7.upload_id AND (
	ll.fuzzy_descriptor_suffix_id = l7.id OR (ll.fuzzy_descriptor_suffix_id IS NULL AND ll.descriptor_suffix_id = l7.id)
)
